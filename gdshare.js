...
function getLevels(data, callback = null) {
    ...
    if (levelList) {
        levelList.forEach((lvl, index) => {
            ...
            callback(n, index+1, levelList.length);
        });
    }
...
function progress(msg, per) {
    process.stdout.clearLine();
    process.stdout.cursorTo(0);
    process.stdout.write(`${msg}... ${per}`);
}
...
function decodeCCFile(path) {
    ...
    return new Promise((res, rej) => {
        ...
        if (!saveData.startsWith('<?xml version="1.0"?>')){
            ...
            try {
                let decode = saveData.split("").map(str => String.fromCharCode(
                    11 ^ str.charCodeAt(0)
                )).join("").replace(/-/g, "+").replace(/_/g, "/");
                progress(dm,"40%");

                decode = Buffer.from(decode, "base64");
                progress(dm,"60%");

                decode = pako.inflate(decode);
                progress(dm,"80%");

                saveData = new TextDecoder("utf-8").decode(decode);
                progress(dm, "100%");
            ...
            progress(dm, "100% (Already decoded)\n");
            res(saveData);
        }
    });
}
...
function getLevelInfo(name, from = "") {
    ...
    return new Promise((res, rej) => {
        ...
        if (from){
            foundLevel = from.find(x => x.name.toLowerCase().trim() === name.toLowerCase().trim());
        ...
        if (!foundLevel){
            ...
            levelInfo["objcount"] = getKey(foundLevel, "k48", "i", 1);
            levelInfo["editortime"] = time > 3600 ? (time/3600).toFixed(1) + "h" : (time/60).toFixed(1) + "m";
            ...
            levelInfo["copy"] = copy === "" ? "None" : copy;
            levelInfo["data"] = getKey(foundLevel, "k4", "s", 1);
